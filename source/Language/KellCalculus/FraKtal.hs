...
instance SubPattern P' where
    ...
    matchR (P'Message a p') (Message b p NullProcess) =
        combine <$> sequence [Just (Substitution (Map.singleton a b) Map.empty),
                              matchR p' p]
    ...
    matchR (P'BoundComplement m a p') (Message b p NullProcess) =
        if a /= b
        then combine <$> sequence [Just (Substitution (Map.singleton m b) Map.empty),
                                   matchR p' p]
        else Nothing
    matchR _ _ = Nothing
...
instance SubPattern P where
    ...
    matchR p q = let subst = (match (toPattern p) (toAnnotatedMessages q)) in
                 if Set.null subst
                 then Nothing
...
instance Pattern FraKtal where
    ...
    boundNames (J (JMessage _ a p)) =
        ...
        where boundNamesP (P'Variable _) = (∅)
    ...
    boundVariables (J (JMessage _ _ p)) =
        boundVariablesP p
        where boundVariablesP (P'Variable x) = Set.singleton x
