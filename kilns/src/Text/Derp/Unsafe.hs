...
--
-- > eps "foo"
parser :: (Ord t) => ParserRec Parser t a -> FPValue Bool -> FPValue Bool -> Parser t a
...
deriveImp p' x' = deriveImpRec (parserRec p') x'
  where
    ...
    deriveImpRec (Ter c) (Token x t) | x `Set.member` c = eps t | True = emp
